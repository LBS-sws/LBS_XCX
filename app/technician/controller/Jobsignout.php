...
class Jobsignout
{
    public function index()
    {
        ...
        if ($token==$user_token['token'] &&  ($c_time <= 24)) {
            if($jobtype==1){
                $job_datas = Db::table('joborder')->where('JobID', $jobid)->update(['FinishDate' => $signdate , 'FinishTime' => $starttime]);
            }elseif ($jobtype==2) {
               $job_datas = Db::table('followuporder')->where('FollowUpID', $jobid)->update(['FinishTime' => $starttime]);
            }
            if ($job_datas) {
                //返回数据
                $result['code'] = 1;
