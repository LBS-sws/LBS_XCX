...
class Addequipmentbyscan
{
    public function index()
    {
        ...
        if(!isset($_POST['staffid']) || !isset($token) || !isset($_POST['job_id']) || !isset($_POST['job_type']) || !isset($_POST['scan_code'])){
            return json($result); 
        }
        if(empty($_POST['staffid']) || empty($token) || empty($_POST['job_id']) || empty($_POST['job_type']) || empty($_POST['scan_code'])){
            return json($result); 
        }
        //获取用户登录信息
        ...
        if ($token==$user_token['token'] &&  ($c_time <= 24)) {
            ...
            //查询设备
            $e_type = Db::table('lbs_service_equipment_numbers')->where('equipment_number',$_POST['scan_code'])->find();
            if(!is_null($e_type)){
