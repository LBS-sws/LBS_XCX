...
//        $staff_id = '';
        ...
        foreach ($jobId as $job_id) {
            $AutographV2 = AutographV2::where(['job_id'=>$job_id,'job_type'=>$jobType])->find();
            if($AutographV2){//已存在
                AutographV2::where(['job_id'=>$job_id,'job_type'=>$jobType])->save(['customer_grade'=>$score]);
            }else{//插入
                $data['job_id'] = $job_id;
    ...
    public function getAnswer(): Json{
        ...
        //处理数据
        $data = $this->processData($questions,$questionType);
        return success(1,'成功',$data);
    }
